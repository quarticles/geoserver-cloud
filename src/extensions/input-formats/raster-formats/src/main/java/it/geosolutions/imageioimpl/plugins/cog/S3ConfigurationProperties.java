 ...
 *
 *    Copied from imageio-ext for shadowing S3RangeReader to support S3-compatible endpoints.
 */
    ...
    public S3ConfigurationProperties(String alias, BasicAuthURI cogUri) {
        ...
        if (region == null) {
            throw new RuntimeException(
                    "No region info found for alias " + this.alias + ".  Please set environment variable '"
                            + AWS_S3_REGION_KEY + "' or system property '"
                            + PropertyLocator.convertEnvVarToProperty(AWS_S3_REGION_KEY) + "'");
        }
